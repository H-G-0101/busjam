...
function gamedist_showNoAdPopup(a,b){a=a||"No ad available right now. Please try again later.";b=b||3E3;var e=document.getElementById("gd-no-ad-popup");e||(e=document.createElement("div"),e.id="gd-no-ad-popup",e.style.cssText="position:fixed;left:50%;top:50%;transform:translateX(-50%);z-index:999999;max-width:90vw;padding:16px 20px;border-radius:10px;background:rgba(0,0,0,0.75);color:#fff;font:20px/1.2 sans-serif;text-align:center;pointer-events:none;opacity:0;transition:opacity .15s ease",document.body.appendChild(e));
e.textContent=a;e.style.opacity="1";_gdNoAdPopupTimer&&clearTimeout(_gdNoAdPopupTimer);_gdNoAdPopupTimer=setTimeout(function(){e.style.opacity="0"},b)}
...
function getUnusedSpace(){var a=Resolution.WIDTH,b=Resolution.HEIGHT;const e=engine.getRenderWidth(),f=engine.getRenderHeight();if(e>f){var h=Math.min(e/a,f/b);b*=h;a=(e-a*h)/2;e>1.75*f&&(a*=.87);b=(f-b)/2;return{isLandscape:!0,horizontalFree:Math.floor(a),verticalFreeTop:Math.floor(b),scale:h,renderWidth:e,renderHeight:f}}h=Math.min(e/a,f/b);return{isLandscape:!1,horizontalFree:0,verticalFreeTop:Math.floor((f-b*h)/2),scale:h,renderWidth:e,renderHeight:f}};var POPUP_TRANSITION_DURATION=400,SCENE_TRANSITION_DURATION=250;RewardAds={showRewAd:function(a,b,e,f){const h=b;b=function(){inlHelper.rewardAds._applyRewardedAdCooldownSmart();inlHelper.rewardAds.rewAdTriggered();"function"===typeof h&&h.call(f)};inlHelper.rewardAds.test?(console.warn("TEST ADS ALLOWED !"),inlHelper.rewardAds.showFakeRewAd(b,e,f)):gamedist_showRewAd(b,e,f)}};window.__bjWatchAd=function(cb){try{RewardAds.showRewAd(activeScene.scene,function(){cb&&cb(!0)},function(){cb&&cb(!1)},null)}catch(e){cb&&cb(!1)}};window.__bjAddCoins=function(n){try{screenTopPanel.incCash(n|0)}catch(e){}};window.__bjCoinAd=function(amount,cb){try{RewardAds.showRewAd(activeScene.scene,function(){try{screenTopPanel.incCash(amount|0)}catch(e){}cb&&cb(!0)},function(){cb&&cb(!1)},null)}catch(e){cb&&cb(!1)}};var Languages=function(a){if(null!=Languages.instance)return Languages.instance;Languages.instance=this;Languages.instance.language="en";this.gameTextsParsed=null;this.xml=a;this.gameTextsLists=[];a=this.xml.getElementsByTagName("string");for(var b=0;b<a.length;b++){null==this.gameTextsLists[a.item(b).getAttribute("id")]&&(this.gameTextsLists[a.item(b).getAttribute("id")]=[]);for(var e=0;e<LANGUAGES.length;e++)0<a.item(b).getElementsByTagName(LANGUAGES[e]).length&&(this.gameTextsLists[a.item(b).getAttribute("id")][LANGUAGES[e]]=

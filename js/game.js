...
/* ===== CiDi Ads Bridge (ex-rede de ads externa) ===== *//* SDK da CiDi: app.cidi.games/sdk/cidi-sdk.js -> expoe window.CiDiSDK *//* Mostra rewarded real. onResult(true)=concede premio (success===true) / onResult(false)=sem ad, fechou ou erro. */function showRewardedAd(onResult){var done=!1,fire=function(ok,why){if(done)return;done=!0;try{window.__bjAdLog&&window.__bjAdLog("reward "+(ok?"CONCEDIDO":"sem premio"),!!ok,why||"");}catch(e){}onResult(!!ok);};try{if(window.CiDiSDK&&typeof window.CiDiSDK.showRewardedAd==="function"){try{window.__bjAdLog&&window.__bjAdLog("showRewardedAd() chamado",null,"timeout=180s");}catch(e){}window.CiDiSDK.showRewardedAd({timeout:180000}).then(function(r){var ok=!!(r&&r.success===!0);try{window.__bjAdLog&&window.__bjAdLog("resolved",ok,"raw="+JSON.stringify(r));}catch(e){}fire(ok,ok?"success=true":"success!=true");}).catch(function(e){try{window.__bjAdLog&&window.__bjAdLog("rejected (catch)",false,(e&&(e.code||e.message))?((e.code||"")+" "+(e.message||"")):(""+e));}catch(_){}fire(!1,"rejected");});/* trava de seguranca: se o SDK nao responder, libera o jogo */setTimeout(function(){fire(!1,"safety-timeout 185s");},185000);return;}}catch(e){}/* fallback de teste SO em dev (localhost/file:) — em producao sem SDK = sem premio */if(location.protocol==="file:"||location.hostname==="localhost"||location.hostname==="127.0.0.1"){try{window.__bjAdLog&&window.__bjAdLog("dev fallback (localhost)",true,"concede p/ teste");}catch(e){}fire(!0,"dev");}else{try{window.__bjAdLog&&window.__bjAdLog("CiDiSDK ausente",false,"producao sem SDK/Pi Browser");}catch(e){}fire(!1,"no-sdk");}}/* Shim que mantem os 16 call-sites legados (gdsdk.showAd / preloadAd) funcionando. */window.gdsdk={preloadAd:function(){return Promise.resolve();},showAd:function(type){if(type==="rewarded"){return new Promise(function(resolve,reject){showRewardedAd(function(ok){if(ok){if(gdist_ad_clbck&&gdist_ad_clbck[0]){try{gdist_ad_clbck[0].call(gdist_ad_clbck[1]);}catch(e){}}gdist_ad_clbck=gdist_ad_clbck_fail=!1;resolve();}else{reject(new Error("no-ad"));}});});}/* interstitial: CiDi nao usa -> no-op seguro */return Promise.resolve();}};var gamedist_rewAdAvailable=!1;
